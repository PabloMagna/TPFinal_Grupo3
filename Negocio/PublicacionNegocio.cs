...
namespace Negocio
{
    public class PublicacionNegocio
    {
        ...
        public List<Publicacion> ListarPorUsuarioPerfil(int idUsuario)
        {
                AccesoDatos datos = new AccesoDatos();
                List<Publicacion> lista = new List<Publicacion>();
                try
                {
                    datos.setearConsulta("SELECT ID, Titulo, CONVERT(int, Especie) AS Especie, Raza, Edad, Sexo, IDUsuario, Descripcion, FechaHora, Estado, IDLocalidad, IDProvincia FROM Publicaciones where (Estado = 1 OR Estado = 3 OR Estado = 4 OR Estado = 6)  AND IDUsuario =" + idUsuario);
                    datos.ejecutarLectura();
