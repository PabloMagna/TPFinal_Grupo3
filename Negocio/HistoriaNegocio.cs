...
namespace Negocio
{
    public class HistoriaNegocio
    {
        ...
        public List<Historia> ListarPorUsuarioActivas(int idUsuario)
        {
            AccesoDatos datos = new AccesoDatos();
            List<Historia> lista = new List<Historia>();
            ...
            try
            {
                datos.setearConsulta("select ID, IDUsuario, Descripcion,UrlImagen,FechaHora,Estado from Historias Where IdUsuario = @IdUsuario AND Estado = 1");
                datos.setearParametro("@IdUsuario", idUsuario);
                datos.ejecutarLectura();
